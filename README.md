...
You can deploy this project on Vercel using two different approaches depending on your preference.

### Option A: Separate Vercel Projects (Recommended for Custom Settings)
In this approach, you deploy the React frontend and Express backend as two separate Vercel projects and link them using environment variables.

#### 1. Deploy the Backend Project
1. Import your repository to Vercel.
2. Set the **Root Directory** to `backend`.
3. In the environment variables settings, configure the following:
   - `MONGODB_URI`: `mongodb+srv://TauqeerAhmad:Test123@cluster0.dc5wogl.mongodb.net/sortmyscene?retryWrites=true&w=majority`
   - `JWT_SECRET`: `super_secret_jwt_key_123_abc_xyz`
   - `FRONTEND_URL`: `https://your-frontend-vercel-domain.vercel.app` (This allows CORS requests from your frontend site)
4. Click **Deploy**. Note down the backend URL generated by Vercel (e.g. `https://your-backend-vercel-domain.vercel.app`).

#### 2. Deploy the Frontend Project
1. Import your repository to Vercel.
2. Set the **Root Directory** to `frontend`.
3. In the environment variables settings, configure the following:
   - `VITE_API_URL`: `https://your-backend-vercel-domain.vercel.app/api` (The URL of your deployed Vercel backend, ending with `/api`)
4. Click **Deploy**.

---

### Option B: Unified Single Vercel Project
You can deploy both the React frontend and Express backend **together in a single Vercel project** using root `vercel.json` configuration:

1. Import your repository to Vercel.
2. Keep the **Root Directory** as the repository root (do not change it to `frontend` or `backend`).
3. Add the following **Environment Variables** in the Vercel project settings:
   - `MONGODB_URI`: `mongodb+srv://TauqeerAhmad:Test123@cluster0.dc5wogl.mongodb.net/sortmyscene?retryWrites=true&w=majority`
   - `JWT_SECRET`: `super_secret_jwt_key_123_abc_xyz`
4. Click **Deploy**. Vercel will build both, hosting the frontend statically at the root domain and routing all `/api/*` requests to the Express serverless function on the same domain automatically!
